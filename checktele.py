...
@tepthon.on(events.NewMessage(outgoing=True, pattern=r"\.اليوزرات"))
async def _(event):
    if ispay2[0] == "yes":
...
●━━━━━━━━●
    ...
    ''')
                    ...
                    await tepthon.send_message(event.chat_id, f'''متاح ~~> {username}
    error :
    {str(eee)}''')
...
async def _(event):
    if ispay2[0] == "yes":
        ...
        if msg[0] == "تثبيت":  # تثبيت عدد يوزر قناه
            ...
            @tepthon.on(events.NewMessage(outgoing=True, pattern=r"\.condition Attract"))
            async def _(event):
                if "on" in isauto:
