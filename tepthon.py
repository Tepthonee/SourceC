...
@tepthon.on(events.NewMessage(outgoing=True, pattern=r"\.اذاعة للخاص(?: |$)(.*)"))
...
@tepthon.on(events.NewMessage(outgoing=True, pattern=r"\.توقف_التكرار$"))
...
@tepthon.on(events.NewMessage(outgoing=True, pattern=r"\.اوامري"))
async def _(event):
    await event.edit(commands)


@tepthon.on(events.NewMessage(outgoing=True, pattern=r"\.اوامر"))
async def _(event):
    await event.edit(commands)
...
@tepthon.on(events.NewMessage(outgoing=True, pattern=r"\.م5"))
async def _(event):
    start = datetime.datetime.now()
    await event.edit(sec5)


@tepthon.on(events.NewMessage(outgoing=True, pattern=r"\.الاوامر الخاصة"))
async def _(event):
    if ispay2[0] == 'yes':
...
@tepthon.on(events.NewMessage(outgoing=True, pattern=r"\.البنك"))
async def _(event):
    start = datetime.datetime.now()
    await event.edit("Ok...")
    end = datetime.datetime.now()
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id :
        order = await event.reply('●━━━━━━ @Tepthon ━━━━━━●')
...
async def _(event):
    ...
    if len(list.blocked) == 0:
        razan = await event.edit(f'ليس لديك أي شخص محظور !')
    else:
        unblocked_count = 1
